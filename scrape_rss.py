...
def scrape_joc_rss():
    """Scrape Journal of Commerce RSS feed"""
    feed_url = 'https://www.joc.com/api/rssfeed'
    articles = []
    
    print("Scraping Journal of Commerce RSS feed...")
    
    try:
        feed = feedparser.parse(feed_url)
        print(f"Found {len(feed.entries)} entries from Journal of Commerce")
        ...
        for entry in feed.entries:
            ...
            standardized_pubdate = standardize_date(pubdate, 'Journal of Commerce')
        ...
        print(f"Error scraping Journal of Commerce: {e}")
...
def scrape_gcaptain_rss():
    ...
    print("Scraping gCaptain RSS feed...")
    
    try:
        feed = feedparser.parse(feed_url)
        print(f"Found {len(feed.entries)} entries from gCaptain")
        ...
        for entry in feed.entries:
            ...
            # Standardize the date format
            standardized_pubdate = standardize_date(pubdate, 'gCaptain')
        ...
        print(f"Error scraping gCaptain: {e}")
    
    return articles

def scrape_shipping_watch_rss():
    """Scrape Shipping Watch RSS feed"""
    feed_url = 'https://rss-feed-api.aws.jyllands-posten.dk/shippingwatch.com/latest'
    articles = []
    
    print("Scraping Shipping Watch RSS feed...")
    
    try:
        feed = feedparser.parse(feed_url)
        print(f"Found {len(feed.entries)} entries from Shipping Watch")
        ...
        for entry in feed.entries:
            ...
            # Standardize the date format
            standardized_pubdate = standardize_date(pubdate, 'Shipping Watch')
        ...
        print(f"Error scraping Shipping Watch: {e}")
...
def scrape_all_sources():
    ...
    # Scrape Shipping Watch RSS
    shipping_watch_articles = scrape_shipping_watch_rss()
    all_articles.extend(shipping_watch_articles)
    time.sleep(2)
    ...
    # Scrape Journal of Commerce RSS
    joc_articles = scrape_joc_rss()
    all_articles.extend(joc_articles)
